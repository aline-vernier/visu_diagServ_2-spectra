...
class WINENCERCLED(QWidget):
    ...
    def __init__(self, parent=None, conf=None, name='VISU'):
        ...
        path_focal_sport = "C:/Users/APPLI/Downloads/Focal_spot_laser/TachFocale_0001.TIFF"
        p_loc = pathlib.Path(path_focal_sport)
        im = np.array(Image.open(p_loc)).T
    ...
    def setup(self):
        ...
        self.EnergyLabel = QLabel ('Energy (mJ)')
        self.ELaserBox = QDoubleSpinBox()
        self.ELaserBox.setValue(60)
        self.tempsLAbel = QLabel ('Duration (FWHM fs)')
        self.DureeLaserBox = QDoubleSpinBox()
        self.DureeLaserBox.setValue(28)
        self.DureeLaserBox.setMinimum(1)
        self.labelFluence = QLabel("Intensity (x 10^18 W / cm²)")
        self.FluenceBox = QDoubleSpinBox()
        self.labelPeakPower = QLabel("Peak Power (TW)")
        self.PeakPowerBox = QDoubleSpinBox()
        ...
        grid_layout1.addWidget(self.EnergyLabel, 7, 0)
        grid_layout1.addWidget(self.ELaserBox, 7, 1)
        grid_layout1.addWidget(self.tempsLAbel, 8, 0)
        grid_layout1.addWidget(self.DureeLaserBox, 8, 1)
        grid_layout1.addWidget(self.labelFluence,9, 0)
        grid_layout1.addWidget(self.FluenceBox,9, 1)
        grid_layout1.addWidget(self.labelPeakPower, 10, 0)
        grid_layout1.addWidget(self.PeakPowerBox, 10, 1)
    ...
    def CalculE(self):
        ...
        # size = self.roi1.size()
        laserEnergy = self.ELaserBox.value() * 1e-3 # J
        laserDuration = self.DureeLaserBox.value() * 1e-15 # fwhm - s 
        peakPowerTW = laserEnergy / laserDuration * 1e-12
        self.PeakPowerBox.setValue(peakPowerTW)
        ...
        # print('pixelmicron', pixelX, pixelY)

        # intensity circled:
        pos, size = self.roi1.pos(), self.roi1.size() # px
        radius = size / 2 # px
        area_px2 = np.pi * radius[0] * radius[1]
        area_cm2 = area_px2 / pixelX / pixelY * 1e-8 # area cm-2 in the red circle
        intensity = (laserEnergy / area_cm2 / laserDuration) * 1e-18
        # self.FluenceBox.setValue(intensity)
        
        # disk filtered
        disk = self.roi1.getArrayRegion(self.data, self.imh)
        dataNorm = disk / np.max(disk)
        disk_filtered = np.where(dataNorm > 0.2, 1, 0) # threshold of 20%
        disk_area_px2 = disk_filtered.sum()
        disk_area_cm2 = disk_area_px2 / pixelX / pixelY * 1e-8
        # print(f"disk_area_um = {np.sqrt(disk_area_cm2) * 1e4}")
        intensity_disk = (laserEnergy / disk_area_cm2 / laserDuration) * 1e-18
        # print(f"intensity disk = {intensity_disk}, laserEnergy = {laserEnergy}, laserDuration = {laserDuration}, disk_area_cm2 = {disk_area_cm2}")
        self.FluenceBox.setValue(intensity_disk)

        # intensity fwhm:
        area_cm2 = self.fwhmY * self.fwhmX / pixelX / pixelY * 1e-8 # area fwhm cm-2
        intensity_fwhm = peakPowerTW / area_cm2 * 1e-6
        self.FluenceBox.setValue(intensity_fwhm)

        # intensity:
        area_cm2 = self.fwhmY * self.fwhmX / pixelX / pixelY * 1e-8 * 1.0645**2 #
        intensity_e = laserEnergy / area_cm2 / (laserDuration * 1.0645) * 1e-18
        # self.FluenceBox.setValue(intensity_e)
    ...
    def Display(self, data):
        ...
        self.p1.setAspectLocked(True, ratio=1)
        # self.p1.enableAutoRange(False) # prevent the zoom pattern
